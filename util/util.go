...
) (composite_key string, row_was_found bool, err error) {
...
func GetTableRow (
    ...
    row_value       interface{},
    failure_option  GetTableRow_FailureOption,
) (row_was_found bool, err error) {
    _, row_was_found, err = getTableRowAndCompositeKey(stub, table_name, row_keys, row_value, failure_option)
    return
}
...
func InsertTableRow (
    ...
    composite_key, row_was_found, err := getTableRowAndCompositeKey(stub, table_name, row_keys, old_row_value, DONT_FAIL_IF_MISSING)
    if err != nil {
    ...
    if failure_option == FAIL_BEFORE_OVERWRITE && row_was_found {
        err = fmt.Errorf("InsertTableRow failed because the row existed already and FAIL_BEFORE_OVERWRITE was specified")
        return
    } else if failure_option == FAIL_UNLESS_OVERWRITE && !row_was_found {
...
func DeleteTableRow (
    ...
    // Process the failure_option
    if failure_option == FAIL_IF_MISSING && !row_was_found {
